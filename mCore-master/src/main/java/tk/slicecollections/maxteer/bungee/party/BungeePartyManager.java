...
public class BungeePartyManager {
    ...
    public static BungeeParty findPartyByPlayer(String player) {
        return BUNGEE_PARTIES.stream().filter(bp -> bp.listMembers().stream().anyMatch(pp -> pp.getName().equals(player))).findFirst().orElse(null);
    }
