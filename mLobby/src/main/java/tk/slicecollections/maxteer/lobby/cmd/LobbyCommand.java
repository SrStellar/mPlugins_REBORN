...
import tk.slicecollections.maxteer.lobby.cmd.ml.*;
...
public class LobbyCommand extends Commands {
  ...
  public void perform(CommandSender sender, String label, String[] args) {
    if (!sender.hasPermission("mlobby.cmd.lobby")) {
      sender.sendMessage("§6mLobby v" + Main.getInstance().getDescription().getVersion() + " §bpor §6Maxteer§b.");
      return;
    }
  ...
  private void sendHelp(CommandSender sender, int page) {
    ...
    for (int index = 0; index < commands.size(); index++) {
      ...
      pages.get(currentPage).append("§3/ml " + commands.get(index).getUsage() + " §f- §7" + commands.get(index).getDescription() + "\n");
    }
