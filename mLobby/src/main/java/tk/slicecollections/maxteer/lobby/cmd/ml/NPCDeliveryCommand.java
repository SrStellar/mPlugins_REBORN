...
public class NPCDeliveryCommand extends SubCommand {
  ...
  public void perform(Player player, String[] args) {
    if (args.length == 0) {
      player.sendMessage(" \n§eAjuda\n \n§3/ml npcentregas adicionar [id] §f- §7Adicionar NPC.\n§3/ml npcentregas remover [id] §f- §7Remover NPC.\n ");
      return;
    }
    ...
    if (action.equalsIgnoreCase("adicionar")) {
      if (args.length <= 1) {
        player.sendMessage("§cUtilize /ml npcentregas adicionar [id]");
        return;
      }
      ...
      if (args.length <= 1) {
        player.sendMessage("§cUtilize /ml npcentregas remover [id]");
        return;
      }
